...
\usepackage{textcomp}
\usepackage[scale=1.0]{tgheros}  % Tex Gyre Heros
\usepackage[scaled]{beramono}    % Bitstream Vera Mono
\end{lstlisting}

\small luatex使用の場合，デフォルトのテンプレートでfontspecが呼ばれるが，
そうするとupquoteが有効にならず，コードチャンク中の二重引用符が「”」に
なってしまうようだ。

# 日本語フォントの設定{.fragile}

本文にゴシック体フォントをつかうには，yamlヘッダで
\begin{lstlisting}
mainfont: NotoSansCJKjp
\end{lstlisting}
などとするか（上の例はNotoSansCJKjpフォント使用の場合），

- \small（上のyamlヘッダは，LaTeXソースでは`\setmainfont[]{NotoSansCJKjp}`に変換される。）

あるいは
`in-header`で指定したファイルで，LaTeX環境の日本語フォントを指定するとよいみたい。
\begin{lstlisting}[language=TeX]
